            ...
            <div className="flex justify-center items-center py-6">
              <div className="space-y-2 w-full max-w-sm">
                ...
                <Input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                  placeholder="Email"
                />
                <Button onClick={handleSaveProfile} className="w-full bg-blue-600 text-white hover:bg-blue-700">
        ...
        {user.shift && (
          <div className="pt-2">
            <div className="flex items-center gap-2 mb-2">
              <h3 className="text-base font-semibold text-slate-600">Your shift information</h3>
            </div>
            ...
            <p className="text-sm mt-1">
              <span className="text-teal-500 font-semibold">
                {format(new Date(user.shift.startTime), "HH:mm", { locale: id })}
              </span>{" "}
              -{" "}
              <span className="text-rose-500 font-semibold">
                {format(new Date(user.shift.endTime), "HH:mm", { locale: id })}
              </span>
            </p>
          </div>
        )}
