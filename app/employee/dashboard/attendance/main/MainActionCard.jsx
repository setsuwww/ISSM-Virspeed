...
export function MainActionCard({ icon, title, description, color = "slate", onClick, asLink, href, warning }) {

  const t = attendanceActionTheme[color] ?? attendanceActionTheme.slate

  const content = (
    <div onClick={onClick} 
      className={clsx("group flex items-center gap-4 p-4 rounded-xl border bg-white transition-all cursor-pointer")}
    >
