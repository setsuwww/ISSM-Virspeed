...
export async function middleware(req) {
  ...
  try {
    ...
    if (path.startsWith("/supervisor") && payload.role !== "SUPERVISOR") return NextResponse.redirect(new URL("/forbidden", req.url));
...
export const config = {
  matcher: ["/admin/:path*", "/supervisor/:path*", "/employee/:path*", "/user/:path*"],
};
